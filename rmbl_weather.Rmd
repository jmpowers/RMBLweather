...
* Raw data in metric units (cleaned "FPA" data unavailable) - lots of cleanup needed, no interspersed missing values
...
* Columns renamed according to /data/WRCC/weather_wrcc_metadata.csv
...
* Data for billy's station runs 2009 - present (valid precip data starts in 2015), other stations run 2009 - 2016 and lack precip but have soil moisture
...
#the chunk below loads the WRCC data but is slow
...
#TODO this comment says to "take the mean of the precip rate per hour that is reported every 10 min", but the code sums it and that matches well on the monthly scale with other stations
...
daily_WRCC <- hourly_WRCC %>% group_by(id, date) %>% 
  summarize(precip_mm = sum(precip_mm), #sum the hourly precip rates over each 24 hrs 
            max_av_temp_air_C = max(av_temp_air_C), min_av_temp_air_C = min(av_temp_air_C), across(where(is.numeric), mean)) %>% 
  mutate(year=factor(year(date)), julian=yday(date),
         precip_mm = ifelse(precip_mm>50 | precip_mm < 0, NA, precip_mm)) #cut nonsense precip values >50mm/day or < 0

save(daily_WRCC, file="data/WRCC/daily_WRCC.rda")

#for CORBIL filter(date >= as.Date("2015-07-01"))#when precip starts to have reasonable values, temp starts 2015-10-01.
#no missing precip values in CORBIL - may not have been quality checked.
```

## ESS-DIVE

Weather data from the Gold Link station in Mt. Crested Butte

* Data are QAQC'd, gap-filled, and summarized daily.
...
hourly_GTH161_missing <- hourly_GTH161 %>% group_by(date=date(DATE_TIME)) %>% 
  filter(date < ymd("2012-01-01")) %>% summarize(missing_precip_hrs = sum(is.na(PRECIPITATION)))
ggplot(hourly_GTH161_missing, aes(x=date, y=missing_precip_hrs)) + geom_point()
table(hourly_GTH161_missing$missing_precip_hrs) %>% as.data.frame %>% rename(missing_hours=Var1, num_days=Freq) %>% kable
...
NOAA_stations_CoCoRaHS <- NOAA_stations %>% filter(CoCoRaHS) %>% pull(id)
...
daily_NOAA <- meteo_pull_monitors(NOAA_stations$id, keep_flags = T) %>% 
  ...
  mutate(date = case_when(id %in% NOAA_stations_CoCoRaHS ~ date - days(1), TRUE ~ date)) # CoCoRaHS stations are recorded the next day at 7 am, so set them back a day
```
...
daily_CoCoRaHS <- read_csv("data/NOAA_GHCN/CoCoRaHS_DailyPrecipReports_CO_GN.csv", guess_max=1e4) %>% 
  mutate(across(TotalPrecipAmt:TotalSnowSWE, as.numeric),# sets "trace" measurements (T) to NA
...
daily_CoCoRaHS %>% group_by(StationName, StationNumber) %>% summarize(across(c(Latitude, Longitude), unique)) %>% 
  left_join(read_tsv("data/NOAA_GHCN/CoCoRaHS_Stations_CO_GN.tsv")) %>% 
  mutate(StationNumber = cell_spec(StationNumber, "html", link = Link), .keep="unused") %>%
  select(-c(Type, State, County, OldStationNumber)) %>% 
...
daily_long <- bind_rows( 
  ...
  .id="id") %>% 
  bind_rows(daily_WRCC %>% select(id, date, 
                                  tavg_C = av_temp_air_C, tmax_C = max_av_temp_air_C, tmin_C = min_av_temp_air_C,
                                  prcp_mm = precip_mm)) %>%  
  bind_rows(daily_NOAA %>% select(id, date, tavg_C, tmax_C, tmin_C, prcp_mm)) %>% mutate(id=factor(id))
...
# The datasets summarized to line up with the NOAA CoCoRaHS time window, in wide format
daily_all_7am <- daily_GTH161_NA_7am %>% select(date, prcp_mm) %>% mutate(id="GTH161") %>% 
  bind_rows(daily_NOAA %>% select(id, date, prcp_mm) %>% filter(id %in% NOAA_stations_CoCoRaHS)) %>% 
  pivot_wider(names_from="id", values_from="prcp_mm") %>% arrange(date) %>% 
  rename(NOAA_billy="US1COGN0018", EPA_RsrchMdw="GTH161") %>% 
  mutate(ground_covered = factor(c("smmr","wntr"))[1+date %in% do.call(c, map2(c(groundcover$first_snow, first_snow_2020[1]), c(groundcover$first_0_cm, first_snow_2020[2]), ~ seq(date(.x), date(.y), by="day")))]) 
```

Fill in EPA CASTNET data since 2011 with NOAA precip from billy's NOAA station in Gothic.

* TODO: Need to redo this with the EPA data summed from 7 am to 7 am.
...
* See the regressions under [daily precip 1989-present](#daily-precip-1989-present)
...
stations <- c("EPA_NOAA_filled", "EPA_RsrchMdw","ESSDIVE_GoldLink","WRCC_billy","NOAA_billy")
monthly_all <- daily_all %>% group_by(yr, mo) %>% 
  summarize(across(all_of(stations), ~ ifelse(sum(is.na(.x))>5, NA, sum(.x, na.rm=T))), .groups="drop") %>% arrange(yr, mo) 

save(daily_all, file="data/daily_all.rda")
```
...
all_stations <- bind_rows(
  ...
  mutate(source_name = paste(source, name, sep="_"), 
         ...
         color = c(brewer.pal(9, "Set1"), brewer.pal(6, "Dark2"))) %>% 
...
autoplot(map.latlon) + 
  geom_point(data=filter(all_stations, id != "CORALM"), 
...
# Daily precip 1989-present

Compare precipitation data from the following [sources](#stations):

* EPA Research Meadow (1989-2011)
* billy's NOAA station in Gothic, CRESTED BUTTE 6.2 N, enrolled in CoCoRaHS (2004-present)
* The EPA Research Meadow data extended with billy's NOAA station (EPA/NOAA, 1989-present)
* billy's data on WRCC (2015-present), does not match his NOAA data (likely because of different summation windows)
* Mt. Crested Butte Gold Link station cleaned by ESSDIVE (2011-2020)

The correlation plots show how precipitation measurements are correlated on the daily scale. Since the NOAA_billy data from CoCoRaHS is shifted back a day (collected at 7 am the next day), it doesn't match up with the WRCC_billy data (or correlate well with the other stations), which are summed from midnight to midnight. Taking the [monthly](#monthly-precip-1989-present) sum makes them almost match.
...
daily_all %>% pivot_longer(unname(stations), names_to="station", values_to="precipitation_mm") %>% 
  ...
  geom_point() + facet_wrap(vars(yr), ncol=1) + theme(legend.position = "top", legend.direction="vertical") +
...
filter(daily_all_7am) %>% ggplot(aes(y=EPA_RsrchMdw,x= NOAA_billy, color=ground_covered)) + geom_point(shape=1) + geom_smooth(method="lm", se=F) + geom_abline(intercept=0, slope=1) + labs(color="Snow cover") + 
   stat_poly_eq(formula = y ~ x + 0, aes(label = paste(..eq.label..,"x", ..rr.label.., sep = "~~~")), parse = T)
```
...
Combined EPA Research Meadow / NOAA billy dataset from summers 1990-2020
