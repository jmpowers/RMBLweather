...
daily_NOAA <- meteo_pull_monitors(NOAA_stations$id, keep_flags = T) %>% 
  ...
  mutate(date = case_when(id %in% NOAA_stations_morning ~ date - days(1), TRUE ~ date)) # CoCoRaHS stations are recorded the next day at 7 am, so set them back a day
```
...
### Fill in EPA CASTNET data

EPA precip data from the Research Meadow ends in 2011, so fill in with from billy's NOAA/CoCoRaHs station in Gothic.
* Sum the EPA precipitation data (GTH161) from 7 am to 7 am to match CoCoRaHs period, and count hours before 7am as the previous day
...
* billy's data fills gaps from 2012-present, and small gaps from 2004-2011 (US1COGN0018, summer r = 0.90)
...
daily_filled_7am <- daily_all_7am %>% 
  mutate(EPA_NOAA_source = ifelse(is.na(GTH161), ifelse(!is.na(US1COGN0018), "NOAA_billy", 
                                                               ifelse(!is.na(USC00051959), "NOAA_CB", NA)), "EPA") %>% factor,
...
monthly_filled_7am <- daily_filled_7am %>% group_by(yr = year(date), mo = month(date)) %>% 
  summarize(across(-c(date, ground_covered, EPA_NOAA_source), ~ ifelse(sum(is.na(.x))>5, NA, sum(.x, na.rm=T))), .groups="drop") %>%
  arrange(yr, mo)
...
ggplot(daily_filled_7am %>% mutate(year=year(date)) %>% filter(year>=1989), aes(x=yday(date))) + 
  ...
  geom_point(aes(y=EPA_NOAA_filled, color=EPA_NOAA_source), size=0.5) + 
...
daily_long_7am %>% mutate(yr = year(date)) %>% 
  ...
  geom_point() + facet_wrap(vars(yr), ncol=1) + scale_color_manual(values=stations_pal) + 
  labs(x="Day of year", y = "Precipitation (mm)", color="Station", shape="Snow cover") + 
  theme(legend.position = "top")

filter(daily_all_7am) %>% ggplot(aes(x=GTH161, y= US1COGN0018, color=ground_covered)) + coord_fixed()+
  ...
  labs(color="Snow cover", x=stations_source_name["GTH161"], y=stations_source_name["US1COGN0018"])

filter(daily_all_7am) %>% ggplot(aes(x=GTH161, y= USC00051959, color=ground_covered)) + coord_fixed()+
  geom_point(shape=1) + geom_smooth(method="lm", se=F) + geom_abline(intercept=0, slope=1) + 
   stat_poly_eq(formula = y ~ x + 0, aes(label = paste(..eq.label..,"x", ..rr.label.., sep = "~~~")), parse = T) +
  labs(color="Snow cover", x=stations_source_name["GTH161"], y=stations_source_name["USC00051959"])
...
```{r weather_monthly, eval=FALSE}
#recalculate Campbell and Wendlandt 2013 July average, 1989 - 2006 EPA station
(EPA_89_06_July_avg_mm <- monthly_filled_7am %>% filter(mo==7, yr >= 1989, yr <= 2006) %>% pull(EPA_NOAA_filled) %>% mean)
...
Combined EPA Research Meadow / NOAA  dataset from summers 1990-2020
