...
* Starts in 1989 but precipitation data is missing after 2011-12-31 (not sure why, suspiciously at the end of the year), though temperature and air quality records continue to present
...
# Could be customized to accept a few missing hours
acceptable_NAs <- 0
daily_GTH161_NA <- hourly_GTH161 %>% arrange(DATE_TIME) %>% mutate(date = date(DATE_TIME)) %>% group_by(date) %>% 
    summarize(tavg_C=mean(TEMPERATURE), tmax_C=max(TEMPERATURE), tmin_C=min(TEMPERATURE), 
              prcp_mm = ifelse(sum(is.na(PRECIPITATION)) <= acceptable_NAs, sum(PRECIPITATION, na.rm=T), NA)) 
...
* Stations beginning with US1COGN (Colorado-Gunnison County) are [community CoCoRaHS stations](https://www.cocorahs.org/Stations/ListStations.aspx), including  [billy barr's](https://www.ncdc.noaa.gov/cdo-web/datasets/GHCND/stations/GHCND:US1COGN0018/detail) [station in Gothic](https://www.cocorahs.org/Stations/Station.aspx?StationNumber=CO-GN-18). Unlike all the other datasets, the reported precipitation is from 7 am the previous day to 7 am that day. To handle this discrepancy coarsely, the date is set to the day before.
...
NOAA_stations <- meteo_nearby_stations(lat_lon_df = maxfield_coords, station_data = station_data,
                      radius = 10, var = c("PRCP", "TAVG"))$maxfield %>% #stations within 10 km radius of Maxfield Meadow
...
daily_NOAA <- meteo_pull_monitors(NOAA_stations$id, keep_flags = T) %>% 
  ...
  mutate(date = case_when(str_sub(id) == "US1" ~ date - days(1), TRUE ~ date)) # CoCoRaHS stations are recorded the next day at 7 am
```
...
all_stations %>% mutate(id = cell_spec(id, "html", link = url), .keep="unused") %>%
  select(-c(state, gsn_flag, wmo_id, source_name, short_name, color)) %>% relocate(source) %>% 
...
```{r coverage}
ggplot(daily_long, aes(x=date, y=id, color=prcp_mm)) + geom_tile() +
  scale_y_discrete(labels=stations_source_name) + 
  labs(y="Station",x="Year", color="Daily precipitation (mm)") + 
  theme_minimal() + theme(legend.position = "top")

ggplot(daily_long, aes(x=date, y=id, color=tmax_C)) + geom_tile() +
  scale_y_discrete(labels=stations_source_name) + scale_color_viridis_c(option="inferno") +
  labs(y="Station",x="Year", color="Maximum temperature (C)") + 
  theme_minimal() + theme(legend.position = "top")
```
