...
relative_to_CB %>% pivot_longer(-c(year, ground_covered), names_to="id", values_to="prcp_rel_CB") %>% 
  ggplot(aes(y=stations_source_name[id], x=prcp_rel_CB, color=ground_covered))+ 
  geom_vline(xintercept=1) + geom_boxplot() + scale_x_continuous(breaks=0:6)+
...
ggplot(summary_EPA_NOAA %>% pivot_longer(!year), aes(y=value, x=year))+ 
  ...
  geom_smooth(method="lm", color="black", se=F) + geom_point(color="grey40") + labs(x="Year", y="") + 
  stat_poly_eq(formula = y ~ x, aes(label = paste(..rr.label.., ..p.value.label.., sep = "~~~")), parse = T) +
